...
pub fn exec(path: &Path, argv: [Option<String>; MAXARG]) -> Result<usize> {
    ...
    let exec = || -> Result<usize> {
        ...
        uvm.as_mut().unwrap().copyout(sp, &ustack[0..(argc * 2)])?;
        // now sp = *const [&str]

        // make &[&str] from sp(=*const [&str]) and argc(= slice len) at stack
        ...
        Ok(argc) // this ends up in a0, the first argument to main(args: &[&str])
    };
    res = exec();
